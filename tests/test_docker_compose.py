...
class TestDockerfileCompose:
    ...
    def test_dockerfile_compose(self):
        ...
        assert re.search(r'image:\s+postgres:', docker_compose), \
            'Проверьте, что добавили образ postgres:latest в файл ' \
            'docker-compose.yaml '
        assert re.search(r'image:\s+([a-zA-Z0-9]+)\/([a-zA-Z0-9_-]+)(\:['
                         r'a-zA-Z0-9_-]+)', docker_compose), \
            'Проверьте, что добавили сборку контейнера из образа на вашем ' \
            'DockerHub в файл docker-compose.yaml '
