...
class TestWorkflow:
    ...
    def test_workflow(self):
        try:
            with open(
                    f'{os.path.join(settings.BASE_DIR, "yamdb_workflow.yaml")}'
                    f'', 'r') as f:
                yamdb = f.read()

        except FileNotFoundError:
            assert False, 'Проверьте, что добавили файл yamdb_workflow.yaml ' \
                          'в корневой каталог для проверки '

        assert 'on: [push]' in yamdb, 'Проверьте, что добавили действие при ' \
